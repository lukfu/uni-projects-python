    ...
    for i in range(L):
        for j in range(L):
            if np.random.rand() < mu:  # mutate the strategy with probability mu
                if newstrategies[i, j] == 0:  # change for the binary case and mutation
                    newstrategies[i,j] = N
                newstrategies[i,j] = 0
            else:
...
def parameters_binary():
    ...
    dynamic_S.set(1.55)
    dynamic_R.set(0.85)
    strategies = np.ones((L, L), dtype=int) * N  # always cooperate -> n=N
    strategies[int(L / 2), int(L / 2)] = 0  # always defect -> n=0

    ...
    for i in range(L):
        for j in range(L):
            if np.random.rand() < mu:  # mutate the strategy with probability mu
                ...
                ss = [strategies[i, j], strategies[mind[i], j], strategies[i, mind[j]], strategies[pind[i], j],
                      strategies[i, pind[j]]]
...
def parameters_binary():
    ...
    dynamic_S.set(1.5)
    dynamic_R.set(0.86)
    strategies = np.ones((L, L)) * 0  # always cooperate -> n=N
    #strategies[int(L / 5), int(L / 5)] = 0  # always defect -> n=0
    #strategies[int(L * 2 / 5), int(L * 2 / 5)] = 0
    #strategies[int(L * 3 / 5), int(L * 3 / 5)] = 0
    #strategies[int(L * 4 / 5), int(L * 4 / 5)] = 0
    # for all defect and placing cooperator
    #strategies = np.zeros((L, L))  # everyone always defects
    strategies[int(L / 2), int(L / 2)] = N  # cooperating point
    strategies[int(L / 2-1), int(L / 2-1)] = N
    strategies[int(L / 2-1), int(L / 2)] = N
    strategies[int(L / 2), int(L / 2-1)] = N
    strategies[int(L / 2+1), int(L / 2+1)] = N
    strategies[int(L / 2+1), int(L / 2)] = N
    strategies[int(L / 2), int(L / 2+1)] = N
    strategies[int(L / 2+2), int(L / 2)] = N
    strategies[int(L / 2), int(L / 2+2)] = N
